...
<body>
    ...
    <div id="right-panel" class="right-panel ">
        ...
        <div class="breadcrumbs">
					<div class="col-sm-5">
							...
							<button  type="button"   class="btn btn-info textfont" style="background-color:#b03060;margin: 5px;margin-left:13%;border-radius:5px;border-style:none;" href="#ed" data-toggle="modal">One Bag</button>
					</div>
  ...
  <h5 class="modal-title" id="exampleModalLabel">Enter Bag ID</h5>
  ...
  <span aria-hidden="true">&times;</span>
   ...
   <div class="input-group-addon">Bag ID</div>
   <input type="text"  value="" name="bagidall" class="form-control">
  ...
  <button type="submit" name="ariveallitem" class="btn btn-info textfont">Arrived</button>
  </div>
  </form>
					...
					<form action="activeorder.php" method="POST" style="margin-right:;" enctype="multipart/form-data">
					...
					<option value="conf-tu" >In Transit To Erbil</option >
					...
					<input type="text" class="form-control datetimepicker" placeholder="Value" name="data">
					 ...
					 <button type="submit"  name="filterB" class="btn btn-info textfont" style="background-color:#b03060;margin: 5px;border-radius:5px;border-style:none;">Show</button>
					</form>
				...
				<div class="content mt-3">
					...
					<div class="table-responsive" style="zoom:75%;">
							<?php
						...
						if(isset($_POST['filterB'])){
						  ...
						  $query = "SELECT * FROM ord  where $colomn LIKE '%$data_filter%' AND (`canceled` IS NULL OR `canceled`='') AND (`arrive` IS NULL OR `arrive`='') AND !(`conf` IS NULL OR `conf`='')";
						...
						$query = "SELECT * FROM ord where (`canceled` IS NULL OR `canceled`='') AND (`arrive` IS NULL OR `arrive`='') AND !(`conf` IS NULL OR `conf`='') ";
						$result = mysqli_query($conn,$query);
						}
						?>
									...
									?>
										...
										<!--<img src="<?php echo "../image/".$row['img'];?>" width="100px" alt='your image' /> -->
											<a target="_blank"  href="<?php echo "../image/".$row['img'];?>"><i class="btn btn-info textfont fa fa-eye btn-lg" style="border-radius:10px;border-style: none;background-color:#b03060;"></i></a>
																</td>
																	...
																	<td>
																	    	<?php if ($row['conf-tu']) { 	?>
																		<center>
																		<form action="activebag.php" method="POST" class="float-left" style="margin-right:5px;margin-bottom:11%;" enctype="multipart/form-data">
																		<input type="hidden" name="nid" value="<?php echo $row['bagid'];?>">
																			<button style="border-radius:10px;border-style: none;background-color:#b03060;" type="submit" name="updatebtn" class="btn btn-info textfont btn-lg "><i class="ti-shopping-cart-full"></i></button>
															...
															<?php }?>
																	...
																	<td>
																		...
																		<input type="hidden" name="nid" value="<?php echo $row['oid'];?>">
																			...
																			<button style="border-radius:10px;" type="submit" name="updatebtn" class="btn btn-info textfont">Arrived</button>
                                                                    ...
                                                                    <?php }?>
																		...
																		<input type="hidden" name="nid" value="<?php echo $row['oid'];?>">
																		<center>	<button style="border-radius:10px;" type="submit" name="updatebtncancle" class="btn btn-danger textfont">Cancel</button>
																				</center>
																</form>
																...
																<td>
																	<center>
																		...
																		<?php
																				include('orderlist/editmodal.php');
																		?>
