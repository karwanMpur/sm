...
<body>
  ...
  <?php include('select.php') ?>
				...
				<div class="content mt-3">
					...
					<div class="table-responsive" style="zoom:60%">
						<?php
						...
						if(isset($_POST['filterB'])){
						 ...
						 $query = "SELECT * FROM ord LEFT JOIN cust ON ord.costid = cust.id  where $colomn LIKE '%$data_filter%' AND (!(`canceled` IS NULL OR `canceled`='') OR !(`takeit` IS NULL OR `takeit`='')) ";
						...
						$query = "SELECT * FROM ord LEFT JOIN cust ON ord.costid = cust.id where !(`canceled` IS NULL OR `canceled`='') OR !(`takeit` IS NULL OR `takeit`='') ";
						$result = mysqli_query($conn,$query);
						}
						?>
