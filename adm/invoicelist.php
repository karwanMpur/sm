				...
				<div class="content mt-3">
					...
					<div class="table-responsive" style="zoom:86%">
						...
						if(isset($_POST['filterB'])){
						 ...
						 $query = "SELECT * FROM ord LEFT JOIN cust ON ord.costid = cust.id  where $colomn LIKE '%$data_filter%' AND !(  `comp` IS NULL OR `comp`='') AND !(  `conf` IS NULL OR `conf`='') group BY bagid DESC ";
						...
						$query = "SELECT * FROM ord LEFT JOIN cust ON ord.costid = cust.id where (  `comp` IS NULL OR `comp`='') AND !(  `conf` IS NULL OR `conf`='') group BY ord.bagid DESC ";
						$result = mysqli_query($conn,$query);
						}
						?>
									...
									?>
															<tr>
																	...
																	<td><?php echo $row['iqcomp']."/".$row['iq'];?></td>
																	<td><?php echo $row['total'];?>$</td>
																	<td ><?php echo $row['conf'];?></td>
																	<td><?php
																	$query1 = "SELECT * FROM ord where bagid=$bidf ";
																	$query2 = "SELECT * FROM ord where bagid=$bidf AND !(  `arrive` IS NULL or `arrive`='') ";
