...
<body>
  ...
  <?php include('selectcan.php') ?>
				...
				<div class="content mt-3">
					...
					<div class="table-responsive" style="zoom:65%">
						<?php
						...
						if(isset($_POST['filterB'])){
						 ...
						 $query = "SELECT * FROM ord LEFT JOIN cust ON ord.costid = cust.id  where $colomn LIKE '%$data_filter%' AND (!(`canceled` IS NULL OR `canceled`='')) ";
						...
						$query = "SELECT * FROM ord LEFT JOIN cust ON ord.costid = cust.id where !(`canceled` IS NULL OR `canceled`='')";
						$result = mysqli_query($conn,$query);
						}
						?>
											...
											<a target="_blank"  href="<?php echo "../image/".$row['img'];?>"><i class="btn btn-info textfont fa fa-eye btn-lg" style="border-radius:10px;border-style: none;background-color:#b03060;"></i></a>
																	...
																	if ($cance) {
																		...
																		<input type="hidden" name="nid" value="<?php echo $row['oid'];?>">
																			<button type="submit" name="uncancele" style="border-radius:10px;" class="btn btn-warning textfont"><i class="ti-cloud-up"></i></button>
																...
																<?php 	} ?>
																		...
																		<td>
																				<a class="btn btn-danger" style="border-radius:10px;"  href="#del<?php echo $row['oid'];?>" data-toggle="modal"><i class="fa fa-trash white"></i></a>
																			<?php
																					include('orderlist/deletemodal.php');
																			?>
																	</td>
															</tr>
									<?php
